...
namespace Class
{
    public class ClassHazop
    {
        ...
        private string query_table_store_location_in_doc()
        {
            return @" EPHA_M_COMPANY c
                     inner join EPHA_M_BUSINESS_UNIT t on c.seq = t.id_company 
                     inner join epha_m_area_complex pa on c.seq = pa.id_company and pa.id = t.id_plant_area
                     inner join EPHA_M_AREA a on a.id = pa.id_area and  a.id = t.id_area  ";
        }
        ...
        public int get_max(string table_name, string id_pha)
        {
            try
            {
                ...
                DataTable _dt = new DataTable();
                cls = new ClassFunctions();
                string sqlstr = $@" select coalesce(max(id),0)+1 as id from {table_name}";

                if (!string.IsNullOrWhiteSpace(id_pha))
                {
                    sqlstr += " where id_pha = @id_pha";
                    parameters.Add(new SqlParameter("@id_pha", SqlDbType.VarChar) { Value = id_pha });
                }

                _dt = ClassConnectionDb.ExecuteAdapterSQLDataTable(sqlstr, parameters);

                return Convert.ToInt32(_dt.Rows[0]["id"]?.ToString() ?? "-1");
            }
            catch (Exception ex)
            {
        ...
        {
            string[] xsplit = ("workstep,taskdesc,potentailhazard,possiblecase,existing_safeguards,recommendations").Split(',');

            foreach (var col in xsplit)
            {
                #region "History " + col
                {
                    var parameters = new List<SqlParameter>();
                    string sqlstr = $@"select * from (select distinct c.{col} as name
                              from epha_t_header a 
                              inner join EPHA_T_GENERAL b on a.id = b.id_pha 
        ...
        private void _history_whatif(ref DataSet _dsData)
        {
            string[] xsplit = ("list_system,list_sub_system,causes,consequences,existing_safeguards,recommendations").Split(',');
            ...
            foreach (var col in xsplit)
            {
                #region "History " + col
                {
                    ...
                    string sqlstr = $@"select * from (select distinct c.{col} as name
                              from epha_t_header a 
                              inner join EPHA_T_GENERAL b on a.id = b.id_pha 
                              inner join EPHA_T_LIST_WORKSHEET c on a.id = c.id_pha 
                              where a.pha_status not in (81) )t where t.name is not null order by t.name";
        ...
        {
            DataSet dsData = new DataSet();
            int maxRows = param.user_name_list.Count;

            string sqlstr = $"select top {(maxRows == 0 ? 100 : maxRows)}";
            sqlstr += @" seq, seq as id, user_id as employee_id, user_name as employee_name, user_displayname as employee_displayname, user_email as employee_email,
            ...
            {
                var parameters = new List<SqlParameter>();
                string user_name_list = string.Join(",", param.user_name_list.Select((name, index) =>
                {
                    string paramName = $"@userName{index}";
                    parameters.Add(new SqlParameter(paramName, SqlDbType.VarChar, 100) { Value = name });
                    return paramName;
                }));

                sqlstr += $" and user_name in ({user_name_list})";
            }
        ...
        {
            DataSet dsData = new DataSet();
            int maxRows = string.IsNullOrEmpty(param.max_rows) ? 100 : int.Parse(param.max_rows);
            string userIndicator = (param.user_indicator + "").Trim();
            string userFilterText = (param.user_filter_text + "").Trim();
            ...
            {
                cls = new ClassFunctions();
                userIndicator = "sickx" + cls.ChkSqlStr(userIndicator, 4000) + "sickx".Replace("sickx'", "").Replace("'sickx", "");
                userFilterText = "sickx" + cls.ChkSqlStr(userFilterText, 4000) + "sickx".Replace("sickx'", "").Replace("'sickx", "");
            }

            sqlstr = $"select top {maxRows} seq, seq as id, user_id as employee_id, user_name as employee_name, user_displayname as employee_displayname, user_email as employee_email, t.user_title as employee_position, 'assets/img/team/avatar.webp' as employee_img, user_type as employee_type, 0 as selected_type from VW_EPHA_PERSON_DETAILS t where t.user_displayname is not null";
            ...
            else if (!string.IsNullOrEmpty(userIndicator))
            {
                sqlstr += @" and (
            trim(lower(t.user_displayname)) like replace(lower(@userIndicator), ' ', '') or trim(lower(t.user_title)) like replace(lower(@userIndicator), ' ', '') or
            trim(lower(t.user_title + t.user_displayname)) like replace(lower(@userIndicator), ' ', '')
        )";
                parameters.Add(new SqlParameter("@userIndicator", SqlDbType.VarChar, 4000) { Value = "%" + userIndicator + "%" });
            }
            else if (!string.IsNullOrEmpty(userFilterText))
            {
                sqlstr += @" and (
            trim(lower(t.user_displayname)) like replace(lower(@userFilterText), ' ', '') or trim(lower(t.user_title)) like replace(lower(@userFilterText), ' ', '') or
            trim(lower(t.user_title + t.user_displayname)) like replace(lower(@userFilterText), ' ', '')
        )";
                parameters.Add(new SqlParameter("@userFilterText", SqlDbType.VarChar, 4000) { Value = "%" + userFilterText + "%" });
            }
        ...
        private void get_authorization_page(ref DataSet _dsData, string user_name)
        {
        ...
        private void authorization_page_by_doc(ref DataSet _dsData, string user_name)
        {
            ClassLogin clsLogin = new ClassLogin();
            DataTable dtAuthPage = clsLogin._dtAuthorization_Page_By_Doc(user_name, "");
        ...
        private void get_master(ref DataSet _dsData, string sub_software, string page_name)
        {
            ...
            sqlstr = page_name == "followup"
                ? @"SELECT DISTINCT a.id, a.name, a.id AS id_area, a.id AS id_apu, LOWER(a.name) AS area_check, c.seq AS id_company FROM " + query_table_store_location_in_doc() + " ORDER BY a.name"
                : @"SELECT DISTINCT a.id, a.name, a.id AS id_area, a.id AS id_apu, LOWER(a.name) AS area_check FROM " + query_table_store_location_in_doc() + " ORDER BY a.name";
            dt = ClassConnectionDb.ExecuteAdapterSQLDataTable(sqlstr, new List<SqlParameter>());
            dt.TableName = "apu";
            _dsData.Tables.Add(dt.Copy());
            _dsData.AcceptChanges();

            sqlstr = @"SELECT DISTINCT a.name AS id, a.name AS name, c.seq AS id_company FROM " + query_table_store_location_in_doc() + " ORDER BY a.name";
            dt = ClassConnectionDb.ExecuteAdapterSQLDataTable(sqlstr, new List<SqlParameter>());
            ...
            {
                "hazop" => @"SELECT DISTINCT a.id AS id_area, a.id AS id_apu, t.id, t.id, t.descriptions AS name, LOWER(a.name) AS area_check, t.id_company FROM " + query_table_store_location_in_doc() + " ORDER BY t.descriptions",
                "whatif" => @"SELECT DISTINCT a.id AS id_area, a.id AS id_apu, t.id, t.id, t.descriptions AS name, LOWER(a.name) AS area_check, t.id_company FROM " + query_table_store_location_in_doc() + " ORDER BY t.descriptions",
                "jsea" => @"SELECT DISTINCT a.id AS id_area, a.id AS id_apu, t.id, t.id, t.descriptions AS name, LOWER(a.name) AS area_check, t.id_company, t.id_plant_area FROM " + query_table_store_location_in_doc() + " ORDER BY t.descriptions",
                "hra" => @"SELECT DISTINCT a.id AS id_area, a.id AS id_apu, t.id, t.id, (t.descriptions + '-' + pa.name) AS name, LOWER(a.name) AS area_check, t.id_company, t.id_plant_area FROM " + query_table_store_location_in_doc() + " ORDER BY (t.descriptions + '-' + pa.name)",
                _ => @"SELECT DISTINCT a.id AS id_area, a.id AS id_apu, t.id, t.id, t.descriptions AS name, LOWER(a.name) AS area_check, t.id_company, t.id_plant_area FROM " + query_table_store_location_in_doc() + " ORDER BY t.descriptions"
            };
            ...
            var parameters = new List<SqlParameter> { new SqlParameter("@subSoftware", SqlDbType.VarChar, 100) { Value = sub_software.ToLower() } };
            dt = ClassConnectionDb.ExecuteAdapterSQLDataTable(sqlstr, parameters);
            ...
            if (sub_software.ToLower() == "hazop" || sub_software.ToLower() == "whatif")
            {
                ...
                sqlstr = @"SELECT DISTINCT a.id AS id_area, a.id AS id_apu, t.id, t.name + ' : ' + t.descriptions + '(' + CONVERT(VARCHAR, a.name) + '-' + CONVERT(VARCHAR, pa.name) + ')' AS name, LOWER(a.name) AS area_check FROM " + query_table_store_location_in_doc() + " ORDER BY a.id, t.id";
                dt = ClassConnectionDb.ExecuteAdapterSQLDataTable(sqlstr, new List<SqlParameter>());
            ...
            else if (sub_software.ToLower() == "followup" || sub_software.ToLower() == "search")
            {
                ...
                #region master business unit
                sqlstr = @"SELECT DISTINCT a.id AS id_area, a.id AS id_apu, t.id, t.name + ' : ' + t.descriptions + '(' + CONVERT(VARCHAR, a.name) + '-' + CONVERT(VARCHAR, c.plant) + ')' AS name, LOWER(a.name) AS area_check FROM " + query_table_store_location_in_doc() + " ORDER BY a.id, t.id";
                dt = ClassConnectionDb.ExecuteAdapterSQLDataTable(sqlstr, new List<SqlParameter>());
                dt.TableName = "business_unit";
                _dsData.Tables.Add(dt.Copy());
                _dsData.AcceptChanges();
    ...
    {
        new SqlParameter("@sub_software", SqlDbType.VarChar, 50) { Value = sub_software ?? "" }
    };
            ...
            parameters = new List<SqlParameter>
    {
        new SqlParameter("@sub_software", SqlDbType.VarChar, 50) { Value = sub_software ?? "" }
            ...
            sqlstr = @"select distinct t.id, t.descriptions as name, a.id as id_area, a.id as id_apu, lower(a.name) as area_check, t.id_company 
               from " + query_table_store_location_in_doc() + @"
                ...
                parameters = new List<SqlParameter>
        {
            ...
            if (sub_software.ToLower() == "jsea" || sub_software.ToLower() == "followup" || sub_software.ToLower() == "search")
            {
                ...
                parameters = new List<SqlParameter>
        {
            new SqlParameter("@sub_software", SqlDbType.VarChar, 100) { Value = sub_software.ToLower() }
        };
        ...
        public string get_followup(LoadDocModel param)
        {
            ...
            {
                DataSearchFollowUp(ref dsData, user_name, seq, sub_software);
            }
            else
            {
        ...
        public string get_followup_detail(LoadDocFollowModel param)
        {
            try
            {
                ...
                string token_doc = (param.token_doc?.ToString() ?? "").Trim();
                string sub_software = (param.sub_software?.ToString() ?? "").Trim();
                string pha_no = (param.pha_no?.ToString() ?? "").Trim();
                string responder_user_name = (param.responder_user_name?.ToString() ?? "").Trim();
        ...
        public string get_details(LoadDocModel param)
        {
            ...
            string token_doc = param.token_doc?.ToString().Trim() ?? "";
            string sub_software = param.sub_software?.ToString().Trim() ?? "";
            string type_doc = param.type_doc?.ToString().Trim() ?? ""; // review_document
            string seq = token_doc;
            string document_module = param.document_module?.ToString().Trim() ?? "";
            string msg = "";
            ...
            {
                json =   $"{ex.Message.ToString()}-->**msg**{msg}-->**sqlstr**{sqlstr}";
            }
        ...
        public void DataFlow(ref DataSet dsData, string user_name, string seq, string sub_software, string document_module)
        {
            ...
            {
                if (seq == null) { seq = "-1"; }
                if ((seq?.ToString().Trim()) == "") { seq = "-1"; }
                WriteLog("seq start function:" + seq);
                ...
                WriteLog("seq:" + seq);
                WriteLog("header:" + sqlstr);

                id_pha = (get_max("epha_t_header", ""));
                WriteLog("id_pha:" + id_pha);
                    ...
                    try
                    {
                        if (dsData.Tables["business_unit"]?.Rows.Count > 0)
                        {
                            ...
                            if (id_unit_no != "")
                            {
                                ...
                                if (dr_unit?.Length > 0) { ref_unit_no_name = dr_unit[0]["name"]?.ToString() ?? ""; }
                ...
                {
                    #region Department, Sections ของ originator
                    ...
                    #endregion Department, Sections ของ originator
                }
                ...
                where a.seq = @seq
                order by a.seq,b.seq,c.seq,d.seq";
                ...
                where a.seq = @seq
                and lower(document_module) = lower('followup')
                order by a.seq,b.seq";

                parameters.Add(new SqlParameter("@seq", SqlDbType.VarChar, 50) { Value = pha_seq ?? "-1" });
                ...
                from epha_t_header a inner join EPHA_T_DRAWING_WORKSHEET b on a.id  = b.id_pha
                where a.seq = @seq
                and lower(document_module) = lower('review_followup')
                order by a.seq,b.seq";
        ...
        private void _recomment_data(string user_name, string seq, int id_pha, ref DataTable dtma, int id_worksheet)
        {
            ...
            sqlstr = @" select r.*, 'update' as action_type, 0 as action_change, 0 as index_rows 
                    from epha_t_header a     
            ...
            sqlstr = @" select rs.*  
                    , 'update' as action_type, 0 as action_change, 0 as index_rows 
                    from epha_t_header a     
                    inner join EPHA_T_TABLE3_WORKSHEET b on a.id  = b.id_pha  
                    inner join EPHA_T_RECOMMENDATIONS r on a.id  = r.id_pha and b.id = r.id_worksheet  
            ...
            sqlstr = @" select f.*  
                    ...
                    where a.seq = @seq 
                    order by a.seq, b.no, r.no, rs.no, f.no ";
        ...
        private void _hazop_data(ref DataSet? dsData, string user_name, string seq, int id_pha, ref DataTable dtma)
        {
            ...
            sqlstr = @" select b.* , 0 as no  
                ...
                where a.seq = @seq
                order by n.no, g.id, b.no, b.causes_no, b.consequences_no, b.category_no";
        ...
        private void _jsea_data(ref DataSet? dsData, string user_name, string seq, int id_pha, ref DataTable dtma)
        {
            ...
            sqlstr = @" select b.* , 'update' as action_type, 0 as action_change
                ...
                where a.seq = @seq
                order by b.workstep_no, b.taskdesc_no, b.potentailhazard_no, b.possiblecase_no, b.category_no";
        ...
        private void _hra_data(ref DataSet? dsData, string user_name, string seq, int id_pha, ref DataTable dtma)
        {
            ...
            if (true)
            {
                ...
                sqlstr = @" select b.* , 'update' as action_type, 0 as action_change, 0 as index_rows
                    ...
                    where a.seq = @seq
                    order by a.seq, b.id_sub_area, b.no";
                ...
                sqlstr = @" select b.* , 'update' as action_type, 0 as action_change, 0 as index_rows 
                    ...
                    where a.seq = @seq
                    order by a.seq, b.id_type_hazard, b.no";
            ...
            if (true)
            {
                ...
                sqlstr = @" select b.* , 0 as no  
                    ...
                    left join VW_EPHA_PERSON_DETAILS vw on lower(b.responder_user_name) = lower(vw.user_name) 
                    where a.seq = @seq
                    order by a.seq, ts.no, hz.no_subareas, hz.no, b.no";
        ...
        private void _whatif_data(ref DataSet? dsData, string user_name, string seq, int id_pha, ref DataTable dtma)
        {
            ...
            sqlstr = @" select b.* , 0 as no  
                ...
                left join VW_EPHA_PERSON_DETAILS vw on lower(b.responder_user_name) = lower(vw.user_name) 
                where a.seq = @seq
                order by n.no, b.no, b.list_system_no, b.list_sub_system_no, b.causes_no, b.consequences_no, b.category_no";
        ...
        public string get_search_details(LoadDocModel param)
        {
            ...
            //authorization_page
            get_authorization_page(ref dsData, user_name);

            //authorization_page_by_doc
            authorization_page_by_doc(ref dsData, user_name);
        ...
        public void DataHazopSearchFollowUp(ref DataSet dsData, string user_name, string seq, string sub_software)
        {
            ...
            DataTable dtma = new DataTable();
            int id_pha = 0;
            user_name = (string.IsNullOrEmpty(user_name) ? "zkuluwat" : user_name);
        ...
        public void DataSearchFollowUp(ref DataSet dsData, string user_name, string seq, string sub_software)
        {
            DataTable dt = new DataTable();
            int id_pha = 0;
            user_name = string.IsNullOrEmpty(user_name) ? "zkuluwat" : user_name;
            ...
            sqlstr_w = @$"SELECT 0 as no, a.pha_sub_software, a.seq as pha_seq, a.pha_no, g.pha_request_name, '' as responder_user_displayname, '' as responder_user_name_check,
                  COUNT(1) as status_total,
                  COUNT(CASE WHEN LOWER(nw.action_status) IN ('closed', 'close with condition') THEN NULL ELSE 1 END) as status_open,
                  COUNT(CASE WHEN LOWER(nw.action_status) IN ('closed', 'close with condition') THEN 1 ELSE NULL END) as status_closed,
                  'worksheet' as data_by, '' as responder_user_name,
                  a.pha_status, CASE WHEN a.pha_status = 13 THEN 'Waiting Follow Up' ELSE 'Waiting Review Follow Up' END as pha_status_name,
                  'update' as action_type, 0 as action_change
                  FROM epha_t_header a
                  INNER JOIN EPHA_T_GENERAL g ON a.id = g.id_pha
                  INNER JOIN {table_worksheet} nw ON a.id = nw.id_pha
                  WHERE a.pha_status IN (13, 14) AND a.seq IN (SELECT MAX(seq) FROM vw_epha_max_seq_by_pha_no GROUP BY pha_no)";

            if (!string.IsNullOrEmpty(seq)) { sqlstr_w += " AND a.seq = @seq"; }
            if (role_type != "admin") { sqlstr_w += " AND (a.pha_status IN (13, 14) AND ISNULL(nw.responder_action_type, 0) <> 2)"; }
            if (!string.IsNullOrEmpty(sub_software) && role_type != "admin") { sqlstr_w += " AND LOWER(a.pha_sub_software) = LOWER(@sub_software)"; }

            if (!hra_active || (hra_active && user_name != "" && role_type != "admin"))
            {
                sqlstr_w += " AND nw.responder_user_name IS NOT NULL";
                if (!string.IsNullOrEmpty(user_name) && role_type != "admin") { sqlstr_w += " AND LOWER(nw.responder_user_name) = LOWER(@user_name)"; }
            }
            sqlstr_w += " GROUP BY a.pha_status, a.pha_sub_software, a.seq, a.pha_no, g.pha_request_name";

            if (hra_active && false)
            {
                string responder_user_name_fix = cls.ChkSqlStr(user_name, 50);

                sqlstr_r = @$"SELECT 0 as no, a.pha_sub_software, '' as pha_seq, '' as pha_no, '' as pha_request_name,
                      ISNULL(nw.project_team_text, '') as responder_user_displayname, LOWER(@responder_user_name_fix) as responder_user_name_check,
                      COUNT(1) as status_total,
                      COUNT(CASE WHEN LOWER(nw.action_status) = 'closed' THEN NULL ELSE 1 END) as status_open,
                      COUNT(CASE WHEN LOWER(nw.action_status) = 'closed' THEN 1 ELSE NULL END) as status_closed,
                      'responder' as data_by, @responder_user_name_fix as responder_user_name,
                      a.pha_status, CASE WHEN a.pha_status = 13 THEN 'Waiting Follow Up' ELSE 'Waiting Review Follow Up' END as pha_status_name,
                      'update' as action_type, 0 as action_change
                      FROM epha_t_header a
                      INNER JOIN EPHA_T_GENERAL g ON a.id = g.id_pha
                ...
                if (!string.IsNullOrEmpty(user_name) && role_type != "admin")
                {
                    sqlstr_r += " AND nw.id_tasks IN (SELECT id_tasks FROM epha_t_table2_workers WHERE LOWER(user_name) = LOWER(@user_name))";
                }
                sqlstr_r += " GROUP BY a.pha_status, a.pha_sub_software";

                sqlstr_o = @$"SELECT 0 as no, a.pha_sub_software, a.seq as pha_seq, a.pha_no, g.pha_request_name,
                      ISNULL(nw.project_team_text, '') as responder_user_displayname, LOWER(@responder_user_name_fix) as responder_user_name_check,
                      COUNT(1) as status_total,
                      COUNT(CASE WHEN LOWER(nw.action_status) = 'closed' THEN NULL ELSE 1 END) as status_open,
                      COUNT(CASE WHEN LOWER(nw.action_status) = 'closed' THEN 1 ELSE NULL END) as status_closed,
                      ...
                      INNER JOIN EPHA_T_GENERAL g ON a.id = g.id_pha
                      INNER JOIN {table_worksheet} nw ON a.id = nw.id_pha
                      WHERE a.pha_status IN (13) AND a.seq IN (SELECT MAX(seq) FROM vw_epha_max_seq_by_pha_no GROUP BY pha_no)";
                if (!string.IsNullOrEmpty(seq)) { sqlstr_o += " AND a.seq = @seq"; }
                if (role_type != "admin") { sqlstr_o += " AND (a.pha_status IN (13) AND ISNULL(nw.responder_action_type, 0) <> 2)"; }
                if (!string.IsNullOrEmpty(sub_software) && role_type != "admin") { sqlstr_o += " AND LOWER(a.pha_sub_software) = LOWER(@sub_software)"; }
                if (!string.IsNullOrEmpty(user_name) && role_type != "admin")
                {
                    sqlstr_o += " AND nw.id_tasks IN (SELECT id_tasks FROM epha_t_table2_workers WHERE LOWER(user_name) = LOWER(@user_name))";
                }
                sqlstr_o += " GROUP BY a.pha_status, a.pha_sub_software, a.seq, a.pha_no, g.pha_request_name";
            }
            else
            {
                sqlstr_r = @$"SELECT 0 as no, a.pha_sub_software, '' as pha_seq, '' as pha_no, '' as pha_request_name,
                      vw.user_displayname as responder_user_displayname, LOWER(nw.responder_user_name) as responder_user_name_check,
                      COUNT(1) as status_total,
                      COUNT(CASE WHEN LOWER(nw.action_status) = 'closed' THEN NULL ELSE 1 END) as status_open,
                      COUNT(CASE WHEN LOWER(nw.action_status) = 'closed' THEN 1 ELSE NULL END) as status_closed,
                      'responder' as data_by, nw.responder_user_name,
            ...
            if (hra_active && false)
            {
                sqlstr_r = @$"SELECT DISTINCT a.pha_sub_software, a.seq as pha_seq, a.pha_no, g.pha_request_name, a.pha_status,
                      ISNULL(nw.project_team_text, '') as responder_user_displayname,
                      LOWER(nw.responder_user_name) as responder_user_name
                      FROM epha_t_header a
                      INNER JOIN EPHA_T_GENERAL g ON a.id = g.id_pha
                      INNER JOIN {table_worksheet} nw ON a.id = nw.id_pha
            ...
            {
                sqlstr_r = @$"SELECT DISTINCT a.pha_sub_software, a.seq as pha_seq, a.pha_no, g.pha_request_name, a.pha_status,
                      vw.user_displayname as responder_user_displayname,
                      LOWER(nw.responder_user_name) as responder_user_name
                      FROM epha_t_header a
                      INNER JOIN EPHA_T_GENERAL g ON a.id = g.id_pha
                      INNER JOIN {table_worksheet} nw ON a.id = nw.id_pha
        ...
        public string QueryFollowUpDetail(string seq, string pha_no, string responder_user_name, string sub_software, Boolean bOrderBy, ref List<SqlParameter> parameters)
        {
            ...
            else // if (sub_software == "hra")
            {
                sqlstr = @"SELECT 'update' AS action_type, 0 AS action_change, 0 AS responder_active_row,
                    ...
                    0 AS responder_active_row, nw.id_tasks, n.worker_group, '' AS descriptions, sa.sub_area,
                    ...
                    LEFT JOIN VW_EPHA_PERSON_DETAILS vw ON LOWER(nw.responder_user_name) = LOWER(vw.user_name)
                    WHERE nw.responder_comment IS NOT NULL AND nw.id_tasks IS NOT NULL AND a.pha_status IN (13,14) AND a.seq IN (SELECT MAX(seq) FROM vw_epha_max_seq_by_pha_no GROUP BY pha_no)";
            }
            ...
            else // if (sub_software == "hra")
            {
                sqlstr += @" GROUP BY  a.id, UPPER(a.pha_sub_software), a.pha_no, g.pha_request_name,
                    ...
                    hz.type_hazard, hz.health_hazard, hz.health_effect_rating, nw.initial_risk_rating, nw.residual_risk_rating";
        ...
        public string get_hometasks(LoadDocModel param)
        {
            ...
            DataTable dt = DataHomeTask(user_name, seq, sub_software, false, true);
        ...
        public DataTable DataHomeTask(string user_name, string seq, string sub_software, Boolean group_by_user, Boolean all_sub_software, string status_code = "")
        {
            DataTable dt = new DataTable();
            cls = new ClassFunctions();
        ...
        and a.seq in (select max(seq) from vw_epha_max_seq_by_pha_no group by pha_no) and a.pha_status = 13";

            // Add more queries with UNION
            string sqlstr = baseSql + @"
        union
        select a.id as id_pha, a.pha_status,
        ...
        inner join EPHA_T_SESSION s on a.id = s.id_pha
        inner join (select max(id) as id_session, id_pha from EPHA_T_SESSION group by id_pha) s2 on a.id = s2.id_pha and s.id = s2.id_session
        left join (select max(id_session) as id_session, id_pha from EPHA_T_MEMBER_TEAM group by id_pha) t2 on a.id = t2.id_pha and s2.id_session = t2.id_session
        inner join EPHA_T_NODE_WORKSHEET nw on a.id = nw.id_pha
        left join VW_EPHA_PERSON_DETAILS emp on lower(a.pha_request_by) = lower(emp.user_name)
        left join VW_EPHA_PERSON_DETAILS emp_ori on lower(a.pha_request_by) = lower(emp_ori.user_name)
        left join VW_EPHA_PERSON_DETAILS emp_conso on lower(nw.responder_user_name) = lower(emp_conso.user_name)
        where a.pha_status in (14) and nw.responder_user_name is not null and nw.responder_action_date is not null
        and lower(nw.action_status) not in ('closed', 'close with condition')
        and a.seq in (select max(seq) from vw_epha_max_seq_by_pha_no group by pha_no) and a.pha_status = 14";

            // Continue adding more queries with UNION
            sqlstr += @"
        union
        ...
        inner join EPHA_T_APPROVER ta2 on a.id = ta2.id_pha and t2.id_pha = ta2.id_pha and t2.id_session = ta2.id_session";
            ...
            {
                sqlstr += " and isnull(ta2.approver_action_type, 0) <> 2";
            }
        ...
        and a.seq in (select max(seq) from vw_epha_max_seq_by_pha_no group by pha_no)";
        ...
        select distinct a.id as id_pha, a.pha_status,
            ...
            5 as action_sort,
            0 as task, upper(a.pha_sub_software) as pha_type,
            'Assigned' as action_required, a.pha_no as document_number, g.pha_request_name as document_title,
            nw.recommendations_no as rev_def, a.pha_version_text + '(' + a.pha_version_desc + ')' as rev, emp_ori.user_displayname as originator,
            format(nw.responder_action_date, 'dd MMM yyyy') as receivesd, format(g.target_end_date, 'dd MMM yyyy') as due_date,
            null as action_date,
            isnull(datediff(day, getdate(), case when g.target_end_date >= getdate() then g.target_end_date else getdate() end), 0) as remaining,
            emp_ta.user_displayname as consolidator
        from epha_t_header a
        inner join EPHA_T_GENERAL g on a.id = g.id_pha
        inner join EPHA_T_SESSION s on a.id = s.id_pha
        inner join (select max(id) as id_session, id_pha from EPHA_T_SESSION group by id_pha) s2 on a.id = s2.id_pha and s.id = s2.id_session
        ...
        inner join EPHA_T_APPROVER ta2 on a.id = ta2.id_pha and t2.id_pha = ta2.id_pha and t2.id_session = ta2.id_session";
            ...
            sqlstr += @"
        inner join EPHA_T_APPROVER_TA3 ta3 on a.id = ta3.id_pha and ta2.id = ta3.id_approver
        left join VW_EPHA_PERSON_DETAILS emp on lower(ta3.user_name) = lower(emp.user_name)
        left join VW_EPHA_PERSON_DETAILS emp_ori on lower(a.pha_request_by) = lower(emp_ori.user_name)
        left join VW_EPHA_PERSON_DETAILS emp_conso on lower(nw.responder_user_name) = lower(emp_conso.user_name)
        left join VW_EPHA_PERSON_DETAILS emp_ta on lower(ta2.user_name) = lower(emp_ta.user_name)
        where a.pha_status in (21) and a.request_approver = 1
        and a.seq in (select max(seq) from vw_epha_max_seq_by_pha_no group by pha_no)";

            if (all_sub_software)
            {
                string _sqlstr = sqlstr;
                _sqlstr += " union " + sqlstr.Replace("inner join EPHA_T_NODE_WORKSHEET", "left join EPHA_T_TASKS_WORKSHEET");
                _sqlstr += " union " + sqlstr.Replace("inner join EPHA_T_NODE_WORKSHEET", "left join EPHA_T_LIST_WORKSHEET");
                _sqlstr += " union " + sqlstr.Replace("inner join EPHA_T_NODE_WORKSHEET", "left join EPHA_T_TABLE3_WORKSHEET");
            ...
            if (group_by_user == true)
            {
                ...
                if (!string.IsNullOrEmpty(user_name)) { sqlstr += " and lower(t.user_name) = lower(@user_name)"; }
                if (!string.IsNullOrEmpty(seq)) { sqlstr += " and t.id_pha = @seq"; }
                if (!string.IsNullOrEmpty(status_code)) { sqlstr += " and lower(t.pha_status) = lower(@status_code)"; }
            ...
            if (!all_sub_software)
            {
                if (sub_software == "jsea") { sqlstr = sqlstr.Replace("EPHA_T_NODE_WORKSHEET", "EPHA_T_TASKS_WORKSHEET"); }
                if (sub_software == "whatif") { sqlstr = sqlstr.Replace("EPHA_T_NODE_WORKSHEET", "EPHA_T_LIST_WORKSHEET"); }
                if (sub_software == "hra") { sqlstr = sqlstr.Replace("EPHA_T_NODE_WORKSHEET", "EPHA_T_TABLE3_WORKSHEET"); }
            }

            var parameters = new List<SqlParameter>
    {
        new SqlParameter("@user_name", SqlDbType.VarChar, 50) { Value = user_name },
        new SqlParameter("@seq", SqlDbType.VarChar, 50) { Value = seq??"-1" },
        new SqlParameter("@status_code", SqlDbType.VarChar, 50) { Value = status_code }
    };
