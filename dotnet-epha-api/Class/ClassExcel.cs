...
namespace Class
{
    public class ClassExcel
    {
                        ...
                        else
                        {
                            if (string.IsNullOrEmpty(ClassFile.check_file_on_server(folder, _file_fullpath_name)))
                            {
                                ...
                                else
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(ClassFile.check_file_on_server(folder.ToLower(), _file_fullpath_name)))
                                    {
                                        msg_error = "Invalid  File Excel."; goto Next_Line;
                                    }
                                    FileInfo template = new FileInfo(_file_fullpath_name);
                                    if (!template.Exists || template.IsReadOnly)
                                    {
                                        msg_error = "File permissions are not correctly set."; goto Next_Line;
                                    }

                                    ExcelPackage.LicenseContext = OfficeOpenXml.LicenseContext.NonCommercial;
                                    using (ExcelPackage excelPackage = new ExcelPackage(_file_fullpath_name))
                                    {
                                ...
                                if (export_type == "pdf")
                                {
                                    try
                                    {
                                        ...
                                        Workbook workbookPDF = new Workbook(_file_fullpath_name);
                                        ...
                                        msg_error = ClassFile.check_format_file_name(_file_fullpath_name_pdf);
                                        if (string.IsNullOrEmpty(msg_error))
                                        {
                                            workbookPDF.Save(_file_fullpath_name_pdf, options);

                                            // ตรวจสอบว่าไฟล์ PDF ถูกสร้างขึ้นจริงหรือไม่
                                            if (File.Exists(_file_fullpath_name_pdf))
                                            {
                                                // ถ้าไฟล์ถูกสร้างสำเร็จ ให้เรียกใช้ฟังก์ชัน add_drawing_to_appendix
                                                msg_error = add_drawing_to_appendix(seq, _file_fullpath_name_pdf, folder);
                                            }
                                            else
                                            {
                                                // ถ้าไฟล์ไม่ถูกสร้าง แสดงข้อผิดพลาด
                                        ...
                                        if (!string.IsNullOrEmpty(_file_fullpath_name))
                                        {
                                            ...
                                            if (!string.IsNullOrEmpty(msg_error))
                                            {
                                                msg_error = $"Failed to chage path Excel to PDF file: {_file_fullpath_name}";
                                            }
                                        }
                                        //replace filename
                ...
                if (!string.IsNullOrEmpty(seq) && !string.IsNullOrEmpty(sub_software))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (string.IsNullOrEmpty(ClassFile.check_file_on_server(folder, _file_fullpath_name)))
                            {
                                msg_error = excel_master_ram(seq, _file_fullpath_name, false, sub_software);
                                if (!string.IsNullOrEmpty(msg_error)) { goto Next_Line; }
                            }
        ...
        public string add_drawing_to_appendix(string seq, string file_fullpath_name, string sub_software)
        {
            ...
            if (dtDrawing != null)
            {
                if (dtDrawing?.Rows.Count > 0)
                {
                    ...
                    if (string.IsNullOrEmpty(ClassFile.check_file_on_server(sub_software, sourceFilePath)))
                    {
                        for (int i = 0; i < dtDrawing?.Rows.Count; i++)
                        {
                            ...
                            if (string.IsNullOrEmpty(_document_file_name))
                            {
                                ...
                                msg_error = ClassFile.check_file_other(_document_file_name, ref pdfToBeAddedFilePath, "");
        ...
        public string excel_master_ram(string seq, string file_fullpath_name, Boolean report_all, string sub_software)
        {
            if (string.IsNullOrEmpty(seq)) { return "Invalid Seq."; }
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(ClassFile.check_file_on_server(sub_software, file_fullpath_name)))
                {
                    return "Invalid  File Excel.";
                }
                FileInfo template = new FileInfo(file_fullpath_name);
                if (!template.Exists || template.IsReadOnly)
                {
                ...
                using (ExcelPackage excelPackage = new ExcelPackage(template))
                {
                    ...
                    string _file_fullpath_name = "";
                    string pictureFilePath = dt.Rows[0]["document_file_name"]?.ToString() ?? "";
                    if (!string.IsNullOrEmpty(pictureFilePath))
                    {
                        msg_error = ClassFile.check_file_other(pictureFilePath, ref _file_fullpath_name, "");
                        if (string.IsNullOrEmpty(msg_error) && !string.IsNullOrEmpty(_file_fullpath_name))
                        {
                            if (string.IsNullOrEmpty(ClassFile.check_file_on_server(sub_software, _file_fullpath_name)))
                            {
                                // Insert the picture
                                var picture = worksheet.Drawings.AddPicture("RAM", new FileInfo(_file_fullpath_name));
        ...
        public string copy_pdf_file(CopyFileModel param)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(file_name) && !string.IsNullOrEmpty(sub_software))
                {
                    ...
                    {
                        msg_error = ClassFile.copy_file_duplicate(file_name, ref _file_download_name, ref _file_fullpath_name, folder);
                        if (string.IsNullOrEmpty(msg_error) || string.IsNullOrEmpty(_file_fullpath_name))
                        {
        ...
        public string excel_hazop_general(string seq, string file_fullpath_name)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(ClassFile.check_file_on_server("jsea", file_fullpath_name)))
                {
                    return "Invalid  File Excel.";
                }
                FileInfo template = new FileInfo(file_fullpath_name);
        ...
        public string excel_hazop_atendeesheet(string seq, string file_fullpath_name)
        {
            ...
            try
            {
                ...
                dtAll = ClassConnectionDb.ExecuteAdapterSQLDataTable("select t.* from  (" + sqlstr + ")t where t.seq = @seq  ", parameters);

                sqlstr = @" select distinct 0 as no, t.user_name, t.user_displayname, '' as company_text from (" + sqlstr + " )t where t.seq = @seq and t.user_name <> '' order by t.user_name";
                ...
                sqlstr = @" select distinct t.seq_session, t.session_no, t.meeting_date from (" + sqlstr + ")t where t.seq = @seq and order by t.session_no ";
                ...
                if (dtAll != null)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(ClassFile.check_file_on_server("hazop", file_fullpath_name)))
                        {
                            return "Invalid  File Excel.";
                        }
                        FileInfo template = new FileInfo(file_fullpath_name);
        ...
        public string excel_jsea_atendeesheet(string seq, string file_fullpath_name)
        {
            ...
            string sqlstr_def = @" select h.seq, s.id_pha, s.seq as seq_session, s.no as session_no
                         , convert(varchar,s.meeting_date,106) as meeting_date
                         , mt.no as member_no, isnull(mt.user_name,'') as user_name, emp.user_displayname
                         from epha_t_header h 
                         inner join EPHA_T_GENERAL g on h.id = g.id_pha 
            ...
            sqlstr = @" select distinct 0 as no, t.user_name, t.user_displayname, '' as company_text from (" + sqlstr_def + " )t where t.seq = @seq and  t.user_name <> '' order by t.user_name";
            parameters = new List<SqlParameter>();
            parameters.Add(new SqlParameter("@seq", SqlDbType.VarChar, 50) { Value = seq ?? "-1" });
            DataTable dtMember = new DataTable();
            dtMember = ClassConnectionDb.ExecuteAdapterSQLDataTable(sqlstr, parameters);

            sqlstr = @" select distinct t.seq_session, t.session_no, t.meeting_date from (" + sqlstr_def + ")t where t.seq = @seq order by t.session_no ";
            parameters = new List<SqlParameter>();
            parameters.Add(new SqlParameter("@seq", SqlDbType.VarChar, 50) { Value = seq ?? "-1" });
            DataTable dtSession = new DataTable();
            dtSession = ClassConnectionDb.ExecuteAdapterSQLDataTable(sqlstr, parameters);
        ...
        public string excel_jsea_worksheet(string seq, string file_fullpath_name, Boolean report_all)
        {
            ...
            sqlstr = @"select h.seq, tr.user_type ,tr.no, tr.user_displayname, tr.user_title, format(tr.date_review,'dd-MMM-yyyy') as reviewer_date
                         from epha_t_header h
                         inner join (
                         select id_pha, id_session, user_type, no, user_displayname, user_title, date_review from  EPHA_T_RELATEDPEOPLE 
                         union  select id_pha, id_session, user_type, no, user_displayname, user_title, date_review   from  EPHA_T_RELATEDPEOPLE_OUTSIDER 
                         ) tr on h.id  = tr.id_pha " +
                      @" union
                         select h.seq, 'reviewer' as user_type ,tr.no, tr.user_displayname, '' as user_title, format(tr.date_review,'dd-MMM-yyyy') as reviewer_date
                         from epha_t_header h
                         inner join EPHA_T_APPROVER tr on h.id  = tr.id_pha 
                         where tr.approver_type = 'approver' and (h.pha_status in (12,13,21,91) and tr.action_review = 2)" +
                         @" union
                         select h.seq, 'approver' as user_type ,tr.no, tr.user_displayname, '' as user_title, format(tr.date_review,'dd-MMM-yyyy') as reviewer_date
                         from epha_t_header h
                         inner join EPHA_T_APPROVER tr on h.id  = tr.id_pha 
                         where tr.approver_type = 'safety'  and (h.pha_status in (12,13,21,91) and tr.action_review = 2 and tr.approver_type = 'approver' )";

            sqlstr = @"select t.* from (" + sqlstr + ")t where t.seq = @seq order by t.user_type, t.no ";
            ...
            //bApproverSafetyReview 
            bApproverSafetyReview = (dtRelatedPeople?.Rows.Count > 0 && dtRelatedPeople.Select("user_type='reviewer'").Length > 0);
                    ...
                    if (true)
                    {
                        ...
                        for (int i = 0; i < dtWorksheet?.Rows.Count; i++)
                        {
                            ...
                            if (true)
                            {
                                ...
                                {
                                    int iMerge = (dtWorksheet.Select("workstep_no=" + dtWorksheet.Rows[i]["workstep_no"] + " and taskdesc_no=" + dtWorksheet.Rows[i]["taskdesc_no"] + "  and potentailhazard_no=" + dtWorksheet.Rows[i]["potentailhazard_no"] + " and potentailhazard_no=" + dtWorksheet.Rows[i]["potentailhazard_no"])).Length;
                                    if (iMerge > 1)
                                    {
                                        try
                                        {
                                ...
                                {
                                    int iMerge = (dtWorksheet.Select("workstep_no=" + dtWorksheet.Rows[i]["workstep_no"] + " and taskdesc_no=" + dtWorksheet.Rows[i]["taskdesc_no"] + " and potentailhazard_no=" + dtWorksheet.Rows[i]["potentailhazard_no"] + " and potentailhazard_no=" + dtWorksheet.Rows[i]["potentailhazard_no"] + " and possiblecase_no=" + dtWorksheet.Rows[i]["possiblecase_no"])).Length;
                                    if (iMerge > 1)
                                    {
                                        try
                                        {
        ...
        public string import_excel_jsea_worksheet(string seq, string file_fullpath_name, ref DataSet _dsData)
        {
            if (string.IsNullOrEmpty(seq)) { return "Invalid Seq."; }
        ...
        public string excel_whatif_general(string seq, string file_fullpath_name)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(ClassFile.check_file_on_server("whatif", file_fullpath_name)))
                {
                    return "Invalid  File Excel.";
                }
                FileInfo template = new FileInfo(file_fullpath_name);
        ...
        public string excel_whatif_atendeesheet(string seq, string file_fullpath_name)
        {
            ...
            try
            {
                ...
                sqlstr = @" select h.seq, s.id_pha, s.seq as seq_session, s.no as session_no
                         , convert(varchar,s.meeting_date,106) as meeting_date
                         , mt.no as member_no, isnull(mt.user_name,'') as user_name, emp.user_displayname
                         from epha_t_header h 
                         inner join EPHA_T_GENERAL g on h.id = g.id_pha 
                ...
                parameters = new List<SqlParameter>();
                DataTable dtAll = new DataTable();
                dtAll = ClassConnectionDb.ExecuteAdapterSQLDataTable("select * from (" + sqlstr + ")t where t.seq = @seq ", parameters);
                ...
                sqlstr = @" select distinct t.seq_session, t.session_no, t.meeting_date from (" + sqlstr + ")t where t.seq = @seq order by t.session_no ";
        ...
        private string hra_query_summary_report()
        {
            ...
            sqlstr = @" select distinct b.responder_user_name, b.xxx";
            sqlstr += @" from epha_t_header a  
        ...
        public string excel_hra_general(string seq, string file_fullpath_name)
        {
            ...
            try
            {
                ...
                sqlstr = "select t.* from (" + query_sqlstr + ")t where t.seq = @seq";
                ...
                if (dtWorkScope != null)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(ClassFile.check_file_on_server("hra", file_fullpath_name)))
                        {
                            return "Invalid  File Excel.";
                        }
                        FileInfo template = new FileInfo(file_fullpath_name);
        ...
        public string excel_hra_worksheet(string seq, string file_fullpath_name, Boolean report_all, Boolean part_recommendations)
        {
            ...
            try
            {
                ...
                #region Summary Risk
                sqlstr = @"  
                        select 1 as sort_by, a.pha_no, a.seq, a.id, 'Low' as risk_type 
                        , sum(case when  lower(nw.initial_risk_rating) = lower('Low')  then 1 else 0 end) as initial_risk_rating
                        , sum(case when  lower(nw.residual_risk_rating) = lower('Low') then 1 else 0 end) as residual_risk_rating  
                        from EPHA_T_HEADER a
                        inner join VW_EPHA_MAX_SEQ_BY_PHA_NO vmax on a.id = vmax.id_pha
                        inner join EPHA_T_TABLE3_WORKSHEET nw on a.id = nw.id_pha 
                        inner join EPHA_T_RECOMMENDATIONS rec on nw.id_pha = rec.id_pha and nw.id = rec.id_worksheet and rec.recommendations is not null 
                        where (lower(nw.initial_risk_rating) = lower('Low') or  lower(nw.residual_risk_rating) = lower('Low'))
                        group by a.pha_no, a.seq, a.id, nw.initial_risk_rating, nw.residual_risk_rating

                        union

                        select 2 as sort_by, a.pha_no, a.seq, a.id, 'Meduim' as risk_type  
                        , sum(case when  lower(nw.initial_risk_rating) = lower('Meduim')   then 1 else 0 end) as initial_risk_rating
                        , sum(case when  lower(nw.residual_risk_rating) = lower('Meduim')  then 1 else 0 end) as residual_risk_rating  
                        from EPHA_T_HEADER a
                        inner join VW_EPHA_MAX_SEQ_BY_PHA_NO vmax on a.id = vmax.id_pha
                        inner join EPHA_T_TABLE3_WORKSHEET nw on a.id = nw.id_pha 
                        inner join EPHA_T_RECOMMENDATIONS rec on nw.id_pha = rec.id_pha and nw.id = rec.id_worksheet and rec.recommendations is not null 
                        where (lower(nw.initial_risk_rating) = lower('Meduim') or  lower(nw.residual_risk_rating) = lower('Meduim'))
                        group by a.pha_no, a.seq, a.id, nw.initial_risk_rating, nw.residual_risk_rating
 
                        union

                        select 3 as sort_by, a.pha_no, a.seq, a.id, 'High' as risk_type 
                        , sum(case when  lower(nw.initial_risk_rating) = lower('High')   then 1 else 0 end) as initial_risk_rating
                        , sum(case when  lower(nw.residual_risk_rating) = lower('High')  then 1 else 0 end) as residual_risk_rating  
                        from EPHA_T_HEADER a
                        inner join VW_EPHA_MAX_SEQ_BY_PHA_NO vmax on a.id = vmax.id_pha
                        inner join EPHA_T_TABLE3_WORKSHEET nw on a.id = nw.id_pha 
                        inner join EPHA_T_RECOMMENDATIONS rec on nw.id_pha = rec.id_pha and nw.id = rec.id_worksheet and rec.recommendations is not null 
                        where (lower(nw.initial_risk_rating) = lower('High') or  lower(nw.residual_risk_rating) = lower('High'))
                        group by a.pha_no, a.seq, a.id, nw.initial_risk_rating, nw.residual_risk_rating
                        ";

                sqlstr = $" select t.* from ({sqlstr}) t where t.seq = @seq";
        ...
        public string export_potential_health_checklist_template(ReportModel param, Boolean res_fullpath = false)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(seq) && !string.IsNullOrEmpty(sub_software))
                {
                    ...
                    if (string.IsNullOrEmpty(seq) || string.IsNullOrEmpty(_file_download_name) || string.IsNullOrEmpty(_file_fullpath_name))
                    { msg_error = "Invalid folder."; }
                    else
                    {
                        // ตรวจสอบว่าไฟล์ที่ได้มามีอยู่จริงหรือไม่
